...
class Decoder(tf.keras.Model):
    def __init__(self):
        ...
        #Layers
        self.deconv1_W = tf.random.normal([self.kernel_size1, 4, self.filter_size1], stddev = 0.1)
        self.deconv1_b = tf.random.normal([self.filter_size1,], stddev = 0.1)
        
        self.deconv2_W = tf.random.normal([self.kernel_size2, self.filter_size1, self.filter_size2], stddev = 0.1)
    ...
    def call(self, encoder_output):
        ...
        output = tf.nn.conv1d_transpose(output, self.deconv1_W, output_shape = [batchSz, int(3*output.get_shape()[1]), self.filter_size1], strides = self.stride1, padding = 'SAME')
        ...
        output = tf.nn.conv1d_transpose(output, self.deconv2_W, output_shape = [batchSz, int(2*output.get_shape()[1]), self.filter_size2], strides = self.stride2, padding = 'SAME')
