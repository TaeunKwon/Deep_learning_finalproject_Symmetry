...
class Encoder(tf.keras.Model):
    def __init__(self):
        ...
        self.filter_size1 = 100
        self.filter_size2 = 100
        self.filter_size3 = 100
        
        self.kernel_size1 = 50
        self.kernel_size2 = 100
        self.kernel_size3 = 100
        ...
        #                                            padding = 'same', activation = tf.keras.layers.LeakyReLU(alpha = 0.2), kernel_initializer=tf.keras.initializers.RandomNormal(stddev=0.1))
        self.Dense1 = tf.keras.layers.Dense(self.Dense_size, activation = tf.keras.layers.LeakyReLU(),dtype = tf.float32)
    ...
    def call(self, pulses):
        ...
        output = self.encoder_maxpool3(output)
        output = self.encoder_maxpool4(output)
...
class Decoder(tf.keras.Model):
    def __init__(self):
        ...
        #Layers
        #self.deconv1_W = tf.Variable(tf.random.normal([self.kernel_size1, self.filter_size1, 100], stddev = 0.1))
        ...
        self.decoder_upsample3 = tf.keras.layers.UpSampling1D(size = self.pool_size3)
        self.decoder_upsample4 = tf.keras.layers.UpSampling1D(size = self.pool_size4)

    def call(self, encoder_output):
        batchSz = tf.shape(encoder_output)[0]
        output = tf.reshape(self.Dense1(encoder_output), (-1,75,100))
        #print("Decoder Input: ",encoder_output.get_shape())
        #output = tf.nn.conv1d_transpose(encoder_output, self.deconv1_W, output_shape = [batchSz, 75, self.filter_size1], strides = self.stride1, padding = 'SAME')
        ...
        #output = tf.nn.conv1d_transpose(output, self.deconv2_W, output_shape = [batchSz, 450, self.filter_size2], strides = self.stride2, padding = 'SAME')
        ...
        output = self.decoder_upsample3(output)
        output = self.decoder_upsample4(output)
        #print("Decoder after upsample2: ",output.get_shape())
        output = tf.nn.conv1d_transpose(output, self.deconv3_W, output_shape = [batchSz, 2700, self.filter_size3], strides = self.stride3, padding = 'SAME')
